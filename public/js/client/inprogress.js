window.addEventListener("DOMContentLoaded", (event) => {
    ...
    let orderInprogressUrl = `${generalUrl}/api/orders/inprogress`;
    ...
    const getDataList = async (refresh = false, currentPage) => {
        try {
            ...
            if (refresh) {
                requestgetDataList = await axios.get(`${generalUrl}/api/orders/inprogress?page=${currentPage}`, config);
            } else {
                requestgetDataList = await axios.get(orderInprogressUrl, config);
            }
